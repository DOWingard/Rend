...
{
/// \brief Callback function which being called in case of error in background thread (watchdog
/// thread).
...
public:
    ...
    /// \brief Getter for server-side license id.
    /// \return Server-side id or 0 if it's not used.
    ...
    /// \brief Getter for company.
    /// \return Information about the company registered on the LicenseSpring backend.
    virtual Company company() const = 0;
    ...
    /// \details Returns cached data from local license, does not perform a request to backend. See
    /// also BaseManager::getProductDetails
    ...
    /// \brief Checker for if the license is activated as part of a license bundle.
    /// \return True if activated as part of a license bundle, false if not.
    virtual bool isBundleActivated() const = 0;
    ...
    /// \details In case trial duration of product changes, this returns the trial duration at time
    /// of license issuing.
    ...
    /// \details Meaningful only for Floating license,
    /// please query this value only after online license check or registration (for Offline
    /// floating).
    ...
    /// \return String of floating client identifier that was set during registration, see
    /// FloatingClient::register.
    ...
    /// \brief Getter for order store ID.
    /// \return String of order store ID.
    ...
    /// \brief Adds user data to the license object. This method allows you to store custom data
    /// securely inside license file.
    /// \param data Key/value entry to be saved.
    /// \param saveLicense Optional parameter indicating whether license should be saved to the file
    /// after modification.
    /// \details To correctly store strings that contain Unicode characters, make sure the string is
    /// encoded in UTF-8, when creating CustomField, for example:
    /// std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(L"示例文本")
    virtual void addUserData(const CustomField &data, bool saveLicense = true) = 0;

    /// \brief Removes user data from the license.
    /// \param key Key of data field that should be removed, if key is empty this method clears
    /// entire user data.
    /// \param saveLicense Optional parameter indicating whether license should be saved to the file
    /// after modification.
    virtual void removeUserData(
        const std::string &key = std::string(), bool saveLicense = true) = 0;
    ...
    /// \brief Checks if negative consumption is allowed for Consumption license.
    /// \details Meaningful only for Consumption license type.
    ...
    /// \return Boolean value indicating if license will be automatically released in the
    /// destructor.
    virtual bool isAutoReleaseSet() const = 0;

    /// \brief Sets auto release flag. Meaningful only for floating license.
    /// \param autoRelease Indicates whether SDK should automatically release floating license in
    /// the destructor.
    virtual void setAutoRelease(bool autoRelease) = 0;

    /// \brief Increase or decrease license consumption by given value.
    /// \param value Consumption value to be added, note it can be negative, so you can decrease
    /// consumption.
    ...
    /// \throw NotEnoughConsumptionException If license run out of resource.
    /// \throw LicenseSpringException If license is not of Consumption type.
    virtual void updateFeatureConsumption(
        const std::string &featureCode, int32_t value = 1, bool saveLicense = true) = 0;
    ...
    /// \return True if maintenance period expired (system date time is later than maintenance
    /// period time), false if not.
    ...
    /// \brief Almost the same as UTC counterpart, but days counter will be changed on local
    /// midnight.
    /// \details For Subscription license it also counts grace period if set.
    ...
    /// \brief License activation date in local time.
    /// \return Time structure of activation date in local time.
    virtual tm activationDate() const = 0;

    /// \brief License activation date in UTC time.
    /// \return Time structure of activation date in UTC time.
    virtual tm activationDateUtc() const = 0;

    /// \brief Check license local without connection to the backend
    /// \param verifySignature Verify LicenseSignature to ensure license is not tampered with.
    ...
    /// \throw SignatureMismatchException If local license signature is not valid
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    /// \throw HardwareKeyException In case the configured hardware key dongle is missing
    ...
    /// \param removeLocalData If true this method will remove local license file and folders.
    /// created by the SDK. If the license was activated as part of a bundle, local data will not be
    /// removed.
    ...
    virtual bool changePassword(
        const std::string &password, const std::string &newPassword) = 0; // may throw exceptions

    /// \brief Online license check, sync the license with the backend, throws exceptions in case of
    /// errors.
    ...
    /// license features.
    /// \return The most recent InstallationFile available for the license (useful for managing
    /// software updates). Returns null in case request failed and grace period started.
    /// \throw LicenseStateException In case license disabled, was reset or expired
    /// \throw DeviceNotLicensedException In case license does not belong to current computer
    /// \throw DeviceBlacklistedException If device has been blacklisted by product manager (for
    /// Floating license)
    ...
    /// \throw UnknownLicenseSpringException In rare case if something went wrong, please contact
    /// support
    virtual InstallationFile::ptr_t check(const InstallFileFilter &filter = InstallFileFilter(),
        bool includeExpiredFeatures = false) = 0;
    ...
    /// If argument value is negative (default) this means do nothing, if value is 0 it means
    /// disable consumption overage.
    /// If value is positive it means allow overage and set max overage to provided value.
    /// \return Bool value indicating if consumption was synchronized. Returns false in case request
    ...
    /// \brief Sync Consumption Feature(s) with the backend, if code not provided sync all
    /// consumption features
    /// \param featureCode Code of the license feature that need to be synchronized. Pass empty
    /// string to sync all consumption features (default).
    /// \return Bool value indicating if consumption was synchronized. Returns false in case request
    /// failed and grace period started.
    /// \throw NotEnoughConsumptionException If consumption license feature run out of resource
    /// \throw InvalidLicenseFeatureException If license feature with given code not found or not of
    /// consumption type
    /// \throw DeviceBlacklistedException If device has been blacklisted by product manager (for
    /// Floating license)
    /// \throw NoInternetException If cannot reach the server
    ...
    /// To correctly send strings that contain Unicode characters, make sure the string is encoded
    /// in UTF-8, for example: std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(
    /// L"示例文本" )
    /// \param name Device variable name
    /// \param value Device variable value
    /// \param saveLicense Optional parameter indicating whether license should be saved localy
    /// after modification
    /// \note There are multiple methods with the same name and different parameters.
    virtual void addDeviceVariable(
        const std::string &name, const std::string &value, bool saveLicense = true) = 0;
    ...
    /// \brief Send current device variables list to the backend (see device variables on the
    /// platform)
    /// \details This method does nothing if local license does not have any variables.
    /// \return Bool value indicating if device variables were sent. Returns false if there are no
    /// device variables or in case request failed and grace period started.
    /// \throw NoInternetException If cannot reach the server
    /// \throw NetworkTimeoutException In case of network timeout
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    /// \throw HardwareKeyException In case hardware key dongle is missing or misconfigured
    ...
    /// \details This method does not throw exceptions if you getting variables list from local
    /// license or during grace period.
    /// \param getFromBackend Indicates if you want to request device variables list from local
    /// license or backend
    ...
    /// \brief Setup and run license watchdog, a background thread which periodically checks the
    /// license.
    /// \details This is especially useful for floating licenses, but can be used for any. Provided
    /// callback will be called in case of errors. Timeout in minutes, 0 means use default (for
    /// Floating license it's floatingTimeout, for others - 1 hour).
    ...
    /// \brief Setup and run feature watchdog, a background thread which periodically checks
    /// floating license features.
    /// \details Useful only for floating features. Provided callback will be called in case of
    /// errors. Timeout in minutes, 0 means use smallest floating timeout between all currently
    /// active floating features.
    /// \throw WatchdogException If cannot start watchdog thread.
    ...
    /// \details In order to keep floating license alive you should check in (register) it
    /// periodically. This is equivalent to check license online See also
    /// License::setupLicenseWatchdog. Alternatively you can register floating license by calling
    /// License::check.
    /// \throw MaxFloatingReachedException If no more floating licenses are available
    ...
    /// \see See also License::setAutoRelease, when set license will be automatically revoked when
    /// destroyed.
    ...
    /// \details Meaningful only for floating licenses. If borrowing succeeded watchdog (if set)
    /// will be stopped and license auto release flag will be set to false. During borrow period
    /// there is no need to register floating license to keep it alive.
    ...
    /// \param password Password for current user required for borrowing. Relevant only for
    /// user-based floating licenses.
    /// \throw NoInternetException If cannot reach the server
    /// \throw NetworkTimeoutException In case of network timeout
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    /// \throw HardwareKeyException In case hardware key dongle is missing or misconfigured
    /// \throw BorrowLicenseException If borrowing is not allowed or provided borrowing period is
    /// bigger than max.
    /// \note There are multiple methods with the same name and different parameters.
    virtual void borrow(
        uint32_t hours, uint32_t days = 0, const std::string &password = std::string()) = 0;

    /// \brief Borrow floating license till provided end date time.
    /// \details Meaningful only for floating licenses. If borrowing succeeded watchdog (if set)
    /// will be stopped and license auto release flag will be set to false.
    /// \param borrowEndDateTime Borrow end date time in UTC in format "%Y-%m-%dT%H:%M:%SZ", for
    /// example "2022-05-28T15:30:00Z" If it's empty the license will be borrowed for max possible
    /// period.
    /// \throw NoInternetException If cannot reach the server
    /// \throw NetworkTimeoutException In case of network timeout
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    ...
    /// bigger than max.
    /// \note There are multiple methods with the same name and different parameters.
    virtual void borrow(const std::string &borrowEndDateTime = std::string(),
        const std::string &password = std::string()) = 0;

    /// \brief Creates offline deactivation file request
    /// \param deactivationRequestFile Deactivation request file path, empty means use default path
    /// (Desktop)
    /// \throw LocalLicenseException Thrown if could not read license file or if local license
    /// corrupted
    virtual std::wstring deactivateOffline(
    ...
    /// product code
    /// \throw DeviceNotLicensedException If license refresh file does not correspond to current
    /// device.
    ...
    /// \details Checks if license belongs to this device and deactivates it. May throw exceptions
    /// same as deactivate method.
    /// \throw ProductMismatchException If license product code does not correspond to configuration
    /// product code
    ...
    /// \throw AirGapActivationException In case confirmation code from the air gap activation
    /// portal is invalid.
    ...
    /// \details If this returns true, the matching algorithm will be stored in the object’s
    /// internal state for use in later operations
    /// \param deviceIDAlgorithm DeviceIDAlgorithm value
    /// \return True if license belongs to given DeviceIDAlgorithm, false otherwise.
    virtual bool isLicenseBelongsToThisDevice(DeviceIDAlgorithm deviceIDAlgorithm) = 0;

    /// \brief Checks if license belongs to any possible DeviceIDAlgorithm.
    /// \details If this returns true, the matching algorithm will be stored in the object’s
    /// internal state for use in later operations
    /// \return True if license belongs to any possible DevideIDAlgorithm, false otherwise.
    ...
    /// \param addToWatchdog True by default, set to false to avoid adding floating feature to
    /// watchdog
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    /// \throw HardwareKeyException In case hardware key dongle is missing or misconfigured
    /// \throw InvalidLicenseFeatureCodeException If feature code is not found on license
    virtual void registerFloatingFeature(
        const std::string &featureCode, bool addToWatchdog = true) = 0;
    ...
    /// \throw FloatingFeatureDeviceNotInUseException If device was never used to check a license
    /// feature
    virtual void releaseFloatingFeature(const std::string &featureCode) = 0;
};

/// \brief Helper function, converts datetime to string using given format. See strftime reference
/// for format details.
std::string LS_API TmToString(
    const tm &dateTime, const std::string &format = std::string("%d-%m-%Y"));

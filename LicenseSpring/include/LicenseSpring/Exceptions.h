...
public:
    LicenseSpringException(
        const std::string &message, LSErrorCode errorCode = LSErrorCode::eGeneralLSError)
        : std::runtime_error(message.c_str()), m_errorCode(errorCode)
    {
    }
...
/// \brief This exception means that SDK cannot reach the server. Because of no internet connection,
/// DNS issues, etc.
...
/// \details See Configuration::setNetworkTimeout() or ExtendedOptions::setNetworkTimeout() for more
/// details.
...
/// \brief Throws in case floating license timeout or borrowing period is expired and it needs to be
/// registered.
...
/// \brief Throws in case computer hardware configuration has been changed or license does not
/// belong to this computer.
...
/// \brief Rare exception, can arise if activation is limited to some device count or license
/// transfer is prohibited.
/// \details In most cases you can get LicenseStateException,
/// LicenseNotFoundException, LicenseNoAvailableActivationsException or
/// CannotBeActivatedNowException if something wrong with license.
...
public:
    LicenseNoAvailableActivationsException(const std::string &message)
        : LicenseActivationException(message)
    {
        m_errorCode = eLicenseNoAvailableActivations;
    }
};
...
{
public:
    AirGapActivationException(const std::string &message) : LicenseActivationException(message)
    {
        m_errorCode = eInvalidConfirmationCode;
    }
};

/// \brief Can be thrown if start date field is set for the license and current date is behind start
/// date
...
{
public:
    CannotBeActivatedNowException(const std::string &message) : LicenseActivationException(message)
    {
        m_errorCode = eCannotBeActivatedNow;
    }
};
...
/// \brief This exceptions means that license product code doesn't correspond to configuration
/// product code.
...
/// \brief Throws when email is missing in API call for user-based product, probably attempt to get
/// trial license without email (user id).
...
/// \brief Please, contact support or try to update SDK if you got this exception. Normally should
/// never happen.
...
/// \details Rare exception, please contact LicenseSpring support if you faced it. Normally should
/// never happen.
...
/// \brief This exception can arise on virtual machine when VM detection is enabled but product
/// policy prohibits running on VM.
...
/// \brief This exception can arise during online check of floating license if reached maximum users
/// count for the license
...
/// \brief This exception can arise during license check if device has been blacklisted by Product
/// manager
...
/// \details Reasons can be different, for example if customer account cannot be found, SSO not
/// enabled or have not been set up.
...
{
public:
    SSOTokenException(const std::string &message) : SSOException(message)
    {
        m_errorCode = eSSOTokenError;
    }
};
...
class LS_API NegativeConsumptionsNotAllowedException : public LicenseSpringException
{
public:
    NegativeConsumptionsNotAllowedException(const std::string &message)
        : LicenseSpringException(message, LSErrorCode::eNegativeConsumptionsNotAllowed)
    {
    }
};

/// \brief This exception can arise if OAuth credentials are invalid, or OAuth is required for
/// authentication
class LS_API OAuthException : public LicenseSpringException
{
public:
...
/// \brief This exception can arise if unable to set up YubiKey
class LS_API HardwareKeyException : public LicenseSpringException
{
public:
...
/// \brief This exception can arise if sending bundle-related requests for non-bundle products
class LS_API ProductNotBundleException : public LicenseSpringException
{
public:
    ProductNotBundleException(const std::string &message)
        : LicenseSpringException(message, LSErrorCode::eProductNotBundle)
    {
    }
};

/// \brief This exception can arise if a bundle-related response does not contain a license that
/// exists locally.
class LS_API BundleMismatchException : public LicenseSpringException
{
public:
    BundleMismatchException(const std::string &message)
        : LicenseSpringException(message, LSErrorCode::eBundleMismatch)
    {
    }
};

/// \brief This exception can arise if a request with a non-existing username is sent to the
/// Floating server v2
class LS_API FloatingUserNotFoundException : public LicenseSpringException
{
public:
    FloatingUserNotFoundException(const std::string &message)
        : LicenseSpringException(message, LSErrorCode::eFloatingUserNotFound)
    {
    }
};
} // namespace LicenseSpring

...
/// \details Used to store information regarding the options related to the application
/// configuration.
...
public:
    ...
    /// \brief Creates shared pointer for configuration object, used for standard authentication
    /// with API and shared keys.
    ...
    /// If you configuring FloatingServer v1.1.9 or earlier provide Product name instead of Product
    /// code
    /// \param appName Name of application
    /// \param appVersion %Version of application
    /// \param extendedOptions ExtendedOptions object to attach to configuration
    /// \throw ConfigurationException If API/shared key are empty or if product code is empty
    static ptr_t Create(const std::string &apiKey, const std::string &sharedKey,
        const std::string &productCode, const std::string &appName, const std::string &appVersion,
        const ExtendedOptions &extendedOptions = ExtendedOptions());

    /// \brief Creates shared pointer for configuration object, used for OAuth authentication
    /// \details Acts as constructor for configuration class.
    /// \param clientId client_id OAuth parameter
    /// \param clientSecret client_secret oauth parameter
    /// \param productCode Product code\n
    /// If you configuring FloatingServer v1.1.9 or earlier provide Product name instead of Product
    /// code
    /// \param appName Name of application
    /// \param appVersion %Version of application
    /// \param extendedOptions ExtendedOptions object to attach to configuration
    /// \throw ConfigurationException If clientId/clientSecret are empty or if product code is empty
    static ptr_t CreateOAuth(const std::string &clientId, const std::string &clientSecret,
        const std::string &productCode, const std::string &appName, const std::string &appVersion,
        const ExtendedOptions &extendedOptions = ExtendedOptions());

    /// \brief Clones the current Configuration object and sets productCode as its product code.
    /// \return Shared pointer to the new Configuration object.
    ptr_t withProductCode(const std::string &productCode);

    /// \brief Checker if authn is done with OAuth.
    ...
    /// \brief Getter for OAuth client ID.
    /// \return String of OAuth client ID.
    const std::string &getClientId() const;

    /// \brief Getter for OAuth client secret.
    /// \return String of OAuth client secret.
    ...
    /// \details Meaningful for user-based auth only. If you don't plan to provide SSO you can
    /// ignore this method.
    ...
    /// \brief This method is obsolete! Return license file path that was set through
    /// ExtendedOptions or empty string.
    /// \details Use BaseManager::dataLocation, BaseManager::licenseFilePath or LicenseFileStorage
    /// instead
    /// \return License file path.
    const std::wstring &getLicenseFilePath() const;

    /// \brief Getter for device id.
    /// \details Either returns device id passed by ExtendedOptions or, if not provided, creates a
    /// unique id for device.
    /// \return String of device id.
    /// \throw LicenseSpringException General LicenseSpring exception, read warning to see possible
    /// reason
    /// \warning Needs at least 2 valid, readable hardware to create the hardware ID. These are your
    /// device's CPU, motherboard and disk.\n May throw an exception if it can only read 1 or less
    /// pieces of listed hardware.
    ...
    /// \brief Getter for DeviceIDAlgorithm.
    /// \return The current device ID algorithm.
    DeviceIDAlgorithm getDeviceIdAlgorithm();
    ...
    /// \brief Checker for whether encrypted license data corruption checking is enabled or not.
    /// \details See ExtendedOptions::isLicenseCorruptionCheckEnabled for more details.
    /// \return True if enabled, false if not.
    ...
    /// \details See ExtendedOptions::isHardwareKeyEnabled for more details.
    /// \return True if enabled, false if not.
    ...
    /// \details If not previously set through ExtendedOptions, default timeout will be 7 seconds.
    /// \return Connect timeout value in seconds.
    long getConnectTimeout() const;

    /// \brief Setter for connect timeout, used for CURLOPT_CONNECTTIMEOUT option
    /// \param timeoutValue Timeout given in seconds, 0 means timeout is 300 seconds
    ...
    /// \details For Windows builds LSVMD.dll should be deployed along with your software if you use
    /// VM detection.
    /// \return True if VM detection is enabled and device is currently using a VM, otherwise false.
    /// \throw VMDNotAvailableException Windows only. In case VM detection dll not found, tempered
    /// or detection failed.
    bool isVM() const;
    ...
    /// \return Shared pointer for CryptoProvider set by vendor or default CryptoProvider if not
    /// set.
    ...
    /// \brief Getter for the target YubiKey serial number configured via ExtendedOptions.
    /// \return String of the target serial number. Empty if not configured.
    const std::string &getHardwareKeyTargetSerial() const;

    /// \brief Getter for the YubiKey PIN configured via ExtendedOptions.
    /// \return String of the PIN. Empty if not configured.

...
public:
    ...
    /// Set it to true for the first call if you use floating licensing in order to properly release
    /// floating license at the app exit. Otherwise this param can be safely ignored, as well as if
    /// you release floating license manualy.
    /// \warning Automatic floating license releasing may still not work if you link to
    /// LicenseSpring dynamically and not do local license check. In this case we recommend you to
    /// release license manualy or you can create you own static object.
    ...
    /// \see BaseManager::getProductDetails
    ProductDetails::ptr_t getProductDetails(
        bool includeLatestVersion, bool includeCustomFields, const std::string &env) const;
    /// \see LicenseManager::getTrialLicense
    const LicenseID &getTrialLicense(
        Customer::ptr_t user = nullptr, const std::string &licensePolicy = std::string()) const;
    /// \see LicenseManager::getTrialLicense
    const LicenseID &getTrialLicense(const std::string &userEmail) const;
    /// \see LicenseManager::getSSOUrl
    const std::string &getSSOUrl(
        const std::string &accountCode = std::string(), bool useAuthCode = true);
    /// \see LicenseManager::getAirGapActivationCode
    const std::string &getAirGapActivationCode(
    ...
    void activateAirGapLicense(const std::string &confirmationCode,
        const std::wstring &activationResponseFile, const std::string &licenseKey,
        uint32_t policyId);
    ...
    /// \see LicenseManager::getUserLicenses
    ...
    /// \see LicenseManager::activateLicense
    void activateLicense(const std::string &authData,
        const std::string &accountCode = std::string(), bool useAuthCode = true);
    ...
    void relinkLicense(
        DeviceIDAlgorithm deviceIdAlgorithm, const std::string &userPassword = std::string());
    /// \see License::check
    InstallationFile::ptr_t checkLicense(
    ...
    /// \see License::borrow
    void borrowFloatingLicense(
        uint32_t hours, uint32_t days = 0, const std::string &password = std::string());
    /// \see License::borrow
    ...
    void updateFeatureConsumption(
        const std::string &featureCode, int32_t value = 1, bool saveLicense = true);
    ...
    /// \see BaseManager::isLicenseFileCorrupted
    ...
    void addDeviceVariable(
        const std::string &name, const std::string &value, bool saveLicense = true);
    /// \see License::addDeviceVariable
    ...
    const std::wstring &createOfflineActivationFile(
    ...
    const std::wstring &deactivateLicenseOffline(
    ...
    /// \see License::isNegativeConsumptionAllowed
    ...
    /// \see License::activationDate
    tm getLicenseActivationDate() const;
    /// \see License::activationDateUtc
    tm getLicenseActivationDateUtc() const;
    ...
    void clearErrorInfo(); // Manually clear error info
    // ------------------------------------------------------------

...
/// \details Used mainly for activating licenses (online/offline/trial). Also useful for getting the
/// SSO URL, changing a user's password, and getting other information such as list of versions for
/// a product, installation files, and license users.
...
public:
    ...
    static LicenseManager::ptr_t create(
    ...
    /// \brief Destroy license manager and its configuration if there is no more references to the
    /// configuration.
    ~LicenseManager() override = default;

    /// \brief Request trial license from LicenseSpring backend.
    /// \details If you provide user (customer) information please note that user's email is
    /// required field.\n Furthermore, if the initial password for the corresponding user's email
    /// has been changed, the license identifier returned will have an empty string for a
    /// password.\n If user is null you will get anonymous trial (available only for key-based
    /// products).
    /// \param user Customer information. Optional param for key-based products, but required for
    /// user-based products.
    /// \param licensePolicy Optional license policy code, if omited the default license policy for
    /// the product will be used.
    /// \return Trial license identifier ready for activation.
    /// \throw TrialNotAllowedException In case getting trial is not allowed by product license
    /// policy
    ...
    /// \warning If the initial password for the corresponding user's email has been changed, the
    /// license identifier returned will have an empty string for a password. The password will need
    /// to be added manually.\n
    /// \note There are multiple methods with the same name and different parameters.
    LicenseID getTrialLicense(
        Customer::ptr_t user = nullptr, const std::string &licensePolicy = std::string());
    ...
    std::string getAirGapActivationCode(
        const std::string &initializationCode, const std::string &licenseKey);

    /// \brief Verify Confirmation code and activate air gap license using given license policy or
    /// policies.
    ...
    /// portal is invalid.
    const License::ptr_t activateAirGapLicense(const std::string &confirmationCode,
        const std::wstring &policyPath, const std::string &licenseKey, uint32_t policyId);

    /// \brief Activate license with given LicenseID.
    /// \details Send activate license request to the backend, creates a local license file if
    /// succesful.
    /// \param licenseID License identifier that needs to be activated
    /// \return Shared pointer to License if activation succeeded, throws exceptions in case of
    /// errors.
    ...
    /// \brief Activate license with given LicenseID and device variables.
    /// \param licenseID License identifier that needs to be activated
    ...
    /// support
    /// \note There are multiple methods with the same name and different parameters.
    const License::ptr_t activateLicense(
        const LicenseID &licenseID, const std::vector<DeviceVariable> &deviceVariables);
    ...
    /// support
    /// \note There are multiple methods with the same name and different parameters.
    const License::ptr_t activateLicense(const std::string &authData,
    ...
    const License::ptr_t activateLicenseOffline(
        const std::wstring &activationResponseFile = std::wstring());

    /// \brief This method helps upgrade to new device id algorithm. It deactivates current license
    /// with old device id and activate with new one. Internet connection is required.
    /// \details Local license is required to call this method. May throw exceptions the same as
    /// ActivateLicense and License::deactivate methods.
    ...
    /// \throw ProductMismatchException If license product code does not correspond to configuration
    /// product code
    /// \throw DeviceNotLicensedException If reactivation is not possible (e.g. license copied from
    /// other device)
    /// \throw OAuthException In case OAuth credentials are invalid or expired
    /// \throw HardwareKeyException In case hardware key dongle is missing or misconfigured
    const License::ptr_t relinkLicense(
        DeviceIDAlgorithm deviceIdAlgorithm, const std::string &userPassword = "");
    ...
    /// \throw UnknownLicenseSpringException In rare case if something went wrong, please contact
    /// support
    std::vector<std::string> getVersionList(
        const LicenseID &licenseID, const InstallFileFilter &filter = InstallFileFilter());

    /// \brief Get InstallationFile information for given license.
    /// \param licenseID License identifier
    /// \param version Desired product (app) version, optional, if not provided then last available
    /// version assumed
    /// \param filter Optional, you can filter install files by environment and channel
    ...
    /// \brief Retrieves all licenses for the provided username and password and the product this
    /// LicenseManager is initialized for.
    /// \param username Username of the user whose licenses will be retrieved
    /// \param password Password of the user whose licenses will be retrieved
    /// \return std::vector of licenses and user information for each license. Returns an empty
    /// vector if username or password are not provided.
    std::vector<UserLicensesData::ptr_t> getUserLicenses(

...
extern "C"
{
    ...
    LS_API typedef struct LSConfiguration
    {
        ...
        void (*setNetworkTimeout)(
            struct LSConfiguration *self, long timeoutValue); // in seconds, 0 means no timeout

        long (*getConnectTimeout)(struct LSConfiguration *self);

        void (*setConnectTimeout)(
            struct LSConfiguration *self, long timeoutValue); // in seconds, 0 means 300 seconds
        ...
        const char *(*getHardwareKeyTargetSerial)(struct LSConfiguration *self);

        const char *(*getHardwareKeyPin)(struct LSConfiguration *self);
        ...
        bool (*isVM)(
            struct LSConfiguration *self); // in case VM detection enabled returns true if app
                                           // currently running in virtualized environment

        const char *(*getDetectedVMName)(
            struct LSConfiguration *self); // return hypervisor name or empty string

    } LSConfiguration;

    LS_API LSConfiguration *CreateLSConfiguration(const char *apiKey, const char *sharedKey,
        const char *productCode, const char *appName, const char *appVersion,
        LSExtendedOptions *extendedOptions);

    LS_API LSConfiguration *CreateLSConfigurationOAuth(const char *clientId,
        const char *clientSecret, const char *productCode, const char *appName,
        const char *appVersion, LSExtendedOptions *extendedOptions);

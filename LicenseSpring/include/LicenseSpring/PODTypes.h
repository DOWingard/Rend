...
enum LSErrorCode
{
    ...
    eProductMismatchError,   ///< License product code doesn't correspond to configuration product
                             ///< code
    ...
    eLicenseSpringInternalError, ///< Internal SDK error, please contact support or try to update
                                 ///< SDK
    eMissingEmailError,   ///< Missing email in API call for user-based product, probably attempt to
                          ///< get trial license with empty email
    eInvalidOrderIDError, ///< Order with specified ID already exists and ShouldAppend flag set to
                          ///< false
    eInvalidLicenseFeatureError,    ///< Feature does not exist or wrong feature type
    eLicenseNoAvailableActivations, ///< The license has already been activated maximum number of
                                    ///< times
    ...
    eDeviceBlacklisted,        ///< Device has been added to the blacklist by admin on LicenseSpring
                               ///< platform
    ...
    eRequestDateHeaderInvalid, ///< Request DateTime header is invalid, please set correct date time
                               ///< on target device,
    ...
    eNegativeConsumptionsNotAllowed, ///< Negative consumptions not allowed
    eUnauthorizedClient,             ///< OAuth client credentials are either expired or wrong
    eOAuthRequired,       ///< OAuth authorization is required instead of api and shared keys
    eOAuthTokenExpired,   ///< Previously fetched OAuth token is expired
    eInvalidClient,       ///< OAuth client doesn't exist
    eOAuthTokenMalformed, ///< OAuth token is malformed
    eHardwareKeyLocal,     ///< Failed to set up YubiKey locally
    eMissingHardwareKeyCertificate, ///< YubiKey certificate missing from License API
    eHardwareKeySignatureMismatch, ///< Signature generated by YubiKey doesn't match it's certificate
    eHardwareKeyRequired, ///< License requires hardware key
    eProductNotBundle, ///< Product is not bundle
    eBundleMismatch, ///< Mismatch between local bundle and bundle-related response
    eFloatingUserNotFound ///< User not found on Floating server v2
};
...
enum DeviceIDAlgorithm
{
    ...
    /// \details Get UUID of the Windows machine using Computer System Product Id through WMI, on
    /// other platforms same as default.
    WinCSProductId,
    /// \brief Alternative device ID algorithm for Windows
    /// \details Get Microsoft Cryptography MachineGuid from Windows registry, on other platforms
    /// same as default.
    WinCryptographyId,
    /// \brief Device ID algorithm with auto-detection of the platform
    /// \details Same as CloudPlatformsId on Azure, AWS, or GCP, otherwise same as Gen2.
    AutoIdGen2,
    /// \brief Device ID algorithm with auto-detection of the platform, meant for Windows cloud
    /// instances

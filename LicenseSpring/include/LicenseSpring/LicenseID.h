...
/// \details For key-based products it is license key, for user-based products it is user
/// credentials - user identifier (typically email) and password.\n Please note: password only
/// required for license activation, other API does not require password. This class does not throw
/// exceptions.
...
public:
    ...
    /// \details LicenseID considered valid if one of the members (key or user) is not empty.
    /// Equivalent of !isEmpty().
    ...
    /// \brief Returns license key or empty string, if key is not set (for example in case of
    /// user-based product).
    /// \return License key or empty string.
    const std::string &key() const;

    /// \brief Returns user identifier or empty string, if user is not set (for example in case of
    /// key-based product).
    ...
    /// \brief Returns server-side license id, or 0 if license is key-based.
    /// \details The server-side license id exists only in case of multiple user-based licenses
    /// assigned to the same user.
    /// \return Integer server-side id.
    uint64_t serverId() const;

    /// \brief Sets server-side license id
    /// \param serverId Server side ID of the license
    ...
    /// \brief Remove trailing whitespaces from the end and beginning of ID (key or user and
    /// password).
    void trim();

    /// \brief Constructs LicenseID with given license key.
    /// \details The ID will be automatically trimmed, no need to call trim manually if you use this
    /// function.
    ...
    /// \return LicenseID object.
    /// \note There are multiple methods with the same name and different parameters.
    static LicenseID fromUser(
        const std::string &user, const std::string &password = std::string(), bool needTrim = true);
    ...
    /// \return LicenseID object.
    /// \note There are multiple methods with the same name and different parameters.
    static LicenseID fromUser(const std::string &user, uint64_t serverId,
        const std::string &password = std::string(), bool needTrim = true);

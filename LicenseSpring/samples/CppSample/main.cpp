...
int main()
{
    ...
    try
    {
        ...
        std::cout << pConfiguration->getAppName() + ' ' << pConfiguration->getAppVersion()
                  << std::endl;
        std::cout << "LicenseSpring SDK version: " << pConfiguration->getSdkVersion() << std::endl;
        std::cout << "LicenseSpring API version: " << pConfiguration->getLicenseSpringAPIVersion()
                  << std::endl;
        ...
        // Uncomment this line to test Floating server
        // return TestFloatingServer( pConfiguration );

        // Try the bundle sample instead of the single-license sample
        bool tryBundleSample = false;
        // Deactivate and remove the license after sample execution
        bool deactivateAndRemove = false;

        // BaseManager::getProductDetails params
        bool includeLatestVersion = true;
        bool includeCustomFields = true;
        std::string env = "";

        std::shared_ptr<BaseSample> sample = nullptr;
        std::string authMethod = "Authorization method: ";
        ProductDetails productDetails;

        if (tryBundleSample)
        {
            auto bundleManager = BundleManager::create(pConfiguration);
            productDetails = bundleManager->getProductDetails(includeLatestVersion, includeCustomFields, env);
            sample.reset(new BundleSample(productDetails.authorizationMethod(), bundleManager));
            authMethod += (productDetails.authorizationMethod() == AuthMethodKeyBased ? "Key-based" : "User-based");
        }
        else
        {
            auto licenseManager = LicenseManager::create(pConfiguration);
            productDetails = licenseManager->getProductDetails(includeLatestVersion, includeCustomFields, env);
            sample.reset(new LicenseSample(productDetails.authorizationMethod(), licenseManager));
            authMethod += (productDetails.authorizationMethod() == AuthMethodKeyBased ? "Key-based" : "User-based");
        }
        std::cout << authMethod << std::endl << std::endl;

        std::cout << "------------- Product info -------------" << std::endl;
        std::cout << "Product name:             " << productDetails.productName() << std::endl;
        std::cout << "Virtual machines allowed: " << productDetails.isVMAllowed() << std::endl;
        std::cout << "Trial allowed:            " << productDetails.isTrialAllowed() << std::endl;
        std::cout << "Metadata:                 " << productDetails.metadata() << std::endl;

        auto timeout = productDetails.floatingLicenseTimeout();
        if (timeout > 0)
            std::cout << "Floating license timeout: " << timeout << " min" << std::endl;

        if (productDetails.isTrialAllowed() && productDetails.trialPeriod() > 0)
        {
            std::string periodStr =
                "Trial period:             " + std::to_string(productDetails.trialPeriod()) + " day";
            if (productDetails.trialPeriod() > 1)
                periodStr += "s";
            std::cout << periodStr << std::endl;
        }

        // Print product-level custom fields if available
        auto dataFields = productDetails.customFields();
        if (!dataFields.empty())
        {
            std::cout << "Custom data fields available for this product:" << std::endl;
...
int TestFloatingServer(LicenseSpring::Configuration::ptr_t config)
{
    // Optionaly you can provide generic user info to config, it will be passed to the
    // FloatingServer
    ...
    if (!serverAddresses.empty() || serverInfo->registrationExpiry() != 0)
    {
        ...
        std::cout << "Client registration expires in " << serverInfo->registrationExpiry()
                  << " minutes\n";
        std::cout << "Servers:\n";
    ...
    std::string
        clientId; // can be anything, e.g. identifier of this user, machine id or app instance id
    ...
    // In order to keep floating license alive automatically in the background thread uncomment the
    // line below
    // LicenseSample::setupAutomaticLicenseUpdates( license );

    // Test license borrowing
    // license->borrow(); // or fc->borrowLicense( clientId );

    BaseSample::PrintLicense(license);
